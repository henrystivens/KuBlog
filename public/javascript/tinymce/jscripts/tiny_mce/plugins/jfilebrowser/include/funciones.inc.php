...
function compara_fechas($fecha1, $fecha2) {
    if (preg_match("/[0-9]{1,2}\/[0-9]{1,2}\/([0-9][0-9]){1,2}/", $fecha1))
        list($dia1, $mes1, $anno1) = split("/", $fecha1);
    if (preg_match("/[0-9]{1,2}-[0-9]{1,2}-([0-9][0-9]){1,2}/", $fecha1))
        list($dia1, $mes1, $anno1) = split("-", $fecha1);
    if (preg_match("/[0-9]{1,2}\/[0-9]{1,2}\/([0-9][0-9]){1,2}/", $fecha2))
        list($dia2, $mes2, $anno2) = split("/", $fecha2);
    if (preg_match("/[0-9]{1,2}-[0-9]{1,2}-([0-9][0-9]){1,2}/", $fecha2))
        list($dia2, $mes2, $anno2) = split("-", $fecha2);

    $dif = mktime(0, 0, 0, $mes1, $dia1, $anno1) - mktime(0, 0, 0, $mes2, $dia2, $anno2);
    return ($dif);
}
...
function compara_fechas3($fecha1, $fecha2) {
    ...
    list($anno1, $mes1, $dia1) = split("-", $fecha_1[0]);
    list($hora1, $min1, $seg1) = split(":", $fecha_1[1]);

    $fecha_2 = split(" ", $fecha2);
    list($anno2, $mes2, $dia2) = split("-", $fecha_2[0]);
    list($hora2, $min2, $seg2) = split(":", $fecha_2[1]);

    $dif = mktime($hora1, $min1, $seg1, $mes1, $dia1, $anno1) - mktime($hora2, $min2, $seg2, $mes2, $dia2, $anno2);
    return ($dif);
}
...
function proxPrevDia($fecha, $tipo = 'pro', $offset = 1) {
    ...
    switch ($tipo) {
        case 'pro': {
                $fecha2 = date("Y-m-d", mktime(0, 0, 0, $mes, ($dia + $offset), $anno));
                //$fecha2 = date($anno.'-'.$mes.'-'.$dia)+1;
            }
            break;
        case 'pre': {
                $fecha2 = date("Y-m-d", mktime(0, 0, 0, $mes, ($dia - $offset), $anno));
...
function validarEmail($valor, $men) {
    if (!ereg("^([a-zA-Z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$", $valor))
        $error = $men;
    return $error;
}
...
function InsertarInfo($tabla, $campos, $valores, $database='', $coneccion='', $ver_query = FALSE) {
    if (!empty($database) && !empty($coneccion)
...
function SeleccionarInfo($tabla, $campo, $database='', $coneccion='', $where = FALSE, $campo_valor_where = '', $orden = FALSE, $orden_campo_AD = '', $limit = FALSE, $limit_valor = '', $show_query = FALSE) {
    ...
    if ($where == TRUE)
        $query .= " WHERE $campo_valor_where";
    if ($orden == TRUE)
        $query .= " ORDER BY $orden_campo_AD";
    if ($limit == TRUE)
        $query .= " LIMIT $limit_valor";
    //echo $query;exit;
    if (!$resultado = mysql_query($query)) {
        echo mysql_error(); //return FALSE;
        echo '<br />' . $query;
